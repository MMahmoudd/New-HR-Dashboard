  ...
  <v-container fluid>
    <v-card>
      <v-card-title>
        ...
        <router-link :to="{ path: '/faqsForm' }" color="primary">
      ...
      >
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <router-link :to="'/faqsForm/' + item.id">
                <v-btn
                  small
                  fab
                  outlined
...
const Services = ServiceFactory.get("faqs");
export default {
  name: "faqs",
  data: (vm) => ({
    search: "",
    ...
    headers: [
      { text: vm.$t("FAQ.id"), sortable: false, value: "id" },
      { text: vm.$t("FAQ.Question"), sortable: false, value: "question" },
      { text: vm.$t("FAQ.Answer"), sortable: false, value: "answer" },
  ...
  methods: {
    ...
    async exportExel() {
      ...
      import("@/vendor/Export2Excel").then((excel) => {
        const tHeader = ["id", "question ", "answer "];
