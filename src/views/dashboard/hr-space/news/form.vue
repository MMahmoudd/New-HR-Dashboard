<template>
  <v-container id="regular-tables" fluid tag="section">
    <v-card class="py-5">
      <v-card-title>
        {{ this.$route.params.id ? $t("news.editnews") : $t("news.addnews") }}
      </v-card-title>
      <template>
...
export default {
  ...
  data: (vm) => ({
    ...
    data: {
      ...
      title: "",
      description: "",
      company_categories: [{ company_id: 0, categories: [] }],
    },
    CompaniesList: [],
    categoriesList: [],
    password: "",
  ...
  methods: {
    ...
    async submitForm() {
      const form = {
        ...
        company_categories: [
          {
            company_id: this.data.company_categories[0].company_id,
            categories: this.data.company_categories[0].categories,
          },
        ],
      };
