  ...
  <v-container fluid>
    <v-card>
      <v-card-title>
        ...
        <router-link :to="{ path: '/TimeForm' }" color="primary">
      ...
      >
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-tooltip bottom>
            <template v-slot:activator="{ on, attrs }">
              <router-link :to="'/TimeForm/' + item.id">
                <v-btn
                  small
                  fab
                  outlined
...
export default {
  ...
  data: (vm) => ({
    ...
    headers: [
      { text: vm.$t("time.id"), sortable: false, value: "id" },
      { text: vm.$t("time.from"), sortable: false, value: "from" },
      { text: vm.$t("time.to"), sortable: false, value: "to" },
      { text: vm.$t("time.days"), sortable: false, value: "days" },
      {
        text: vm.$t("time.areas"),
        sortable: false,
        value: "assigned_areas[0][name]",
      },

      {
        text: vm.$t("departments.company"),
        sortable: false,
        value: "assigned_companies[0][company]",
      },
      {
        text: vm.$t("time.Categories"),
        sortable: false,
        value: "assigned_companies[0][categories]",
      },
      ...
      import("@/vendor/Export2Excel").then((excel) => {
        ...
        const list = List.data.map((item) => {
          return {
            ...
            to: item.to,
            days: item.days,
            areas: item.assigned_areas.name,
