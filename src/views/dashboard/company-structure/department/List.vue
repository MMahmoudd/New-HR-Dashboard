<template>
  <v-container fluid>
    <v-card>
      <v-card-title>
        {{ $t('CompanyStructure.Departments') }}
        <v-spacer />
        <v-spacer />
        <v-btn
          class="mx-1 my-auto"
...
<script>
  ...
  const Services = ServiceFactory.get('department')
  export default {
    name: 'Department',
    data: (vm) => ({
      search: '',
    ...
    methods: {
      ...
      async exportExel () {
        ...
        const tHeader = ['Department Name', 'Company Name']
        const list = List.data.map(item => {
          return { name: item.name, companies: item.companies }
        })
